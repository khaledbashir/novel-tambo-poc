...
export function Sidebar({
  ...
  const [editingItem, setEditingItem] = React.useState<{ type: 'workspace' | 'document', id: string, name: string } | null>(null);
  const [showModal, setShowModal] = React.useState<'workspace' | 'document' | null>(null);
  ...
  return (
    <>
      <div className={cn("w-64 bg-gradient-to-b from-card to-card/95 border-r border-border/50 flex flex-col shadow-lg", className)}>
        {/* Header */}
        <div className="p-6 border-b border-border/50 bg-gradient-to-r from-primary/5 to-primary/10">
          <h2 className="text-xl font-bold bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent">
            SOW Workbench
          </h2>
          <p className="text-xs text-muted-foreground mt-1">Clients & Projects</p>
        </div>
        ...
        <div className="p-4 border-b border-border/50">
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              className="group flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium bg-gradient-to-r from-primary to-primary/90 text-primary-foreground rounded-lg hover:shadow-lg hover:scale-105 transition-all duration-200 disabled:opacity-50 disabled:hover:scale-100"
              title={!selectedWorkspace ? "Select a client first" : "Create new project"}
            >
              <Plus size={16} className="group-hover:rotate-90 transition-transform duration-200" />
              Project
            </button>
          ...
          <div className="p-3">
            {workspaces.length === 0 ? (
              <div className="p-6 text-center">
                ...
                <p className="text-xs text-muted-foreground mb-4">Create a client workspace to organize SOWs</p>
                <button
                  onClick={() => {
                    setShowModal('workspace');
                    setModalInput("");
              ...
              workspaces.map((workspace) => (
                <div key={workspace.id} className="mb-2">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center flex-1 min-w-0">
                        <div className={cn(
                          ...
                          selectedWorkspace === workspace.id ? "bg-primary/20" : "bg-muted"
                        )}>
                          <Folder className={cn(
                            "w-4 h-4",
                            selectedWorkspace === workspace.id ? "text-primary" : "text-muted-foreground"
                          )} />
                        </div>
                        {editingItem?.type === 'workspace' && editingItem.id === workspace.id ? (
                          <input
                            type="text"
                            defaultValue={workspace.name}
                            autoFocus
                            onBlur={(e) => renameWorkspace(workspace.id, e.target.value)}
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
                      ...
                      <div className="flex items-center gap-1 ml-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setEditingItem({ type: 'workspace', id: workspace.id, name: workspace.name });
                          }}
                  ...
                  {selectedWorkspace === workspace.id && documents.length > 0 && (
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center flex-1 min-w-0">
                              <FileText className={cn(
                                ...
                                selectedDocument === document.id ? "text-primary" : "text-muted-foreground"
                              )} />
                              {editingItem?.type === 'document' && editingItem.id === document.id ? (
                                <input
                                  type="text"
                                  ...
                                  onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                      renameDocument(document.id, e.currentTarget.value);
                                    } else if (e.key === 'Escape') {
                                      setEditingItem(null);
                                    }
                                  }}
                                ...
                                <div className="text-xs truncate">{document.name}</div>
                              )}
                            </div>
                            <div className="flex items-center gap-1 ml-2 opacity-0 group-hover:opacity-100 transition-opacity">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setEditingItem({ type: 'document', id: document.id, name: document.name });
                                }}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
          <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-md mx-4 animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-border">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold">
                  {showModal === 'workspace' ? 'Create Client' : 'Create Project'}
                </h3>
                <button
                  onClick={() => {
                    setShowModal(null);
            ...
            <form onSubmit={showModal === 'workspace' ? handleCreateWorkspace : handleCreateDocument} className="p-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-2">
                    {showModal === 'workspace' ? 'Client Name' : 'Project Name'}
                  </label>
                  <input
                    type="text"
                    value={modalInput}
                    onChange={(e) => setModalInput(e.target.value)}
                    placeholder={showModal === 'workspace' ? 'e.g., Acme Corp' : 'e.g., Website Redesign'}
                    autoFocus
