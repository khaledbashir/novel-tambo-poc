  ...
  addStorage() {
    return {
      markdown: {
        ...
        getMarkdown: () => {
          console.warn("getMarkdown called before editor initialization");
          return "";
        },
      },
    };
  ...
  onCreate() {
    ...
    if (this.editor) {
      ...
      try {
        ...
        console.warn("Could not create schema-specific markdown serializer, using default:", error);
        serializer = defaultMarkdownSerializer;
      }
      ...
      try {
        ...
        const safeSerializer = new (MarkdownSerializer as any)(nodes, marks);
        serializer = safeSerializer as MarkdownSerializer;
      } catch (error) {
        console.warn("Failed to augment markdown serializer, using base serializer:", error);
      }
        ...
        serialize: (content: any) => {
          try {
            ...
            try {
              ...
              try {
                const doc: any = content ?? this.editor.state.doc;
      ...
      this.editor.storage.markdown.getMarkdown = () => {
        try {
          ...
          try {
            ...
            try {
              return String(this.editor.state.doc.textContent ?? "");
            } catch {
              return "";
            }
          }
