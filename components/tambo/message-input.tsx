...
const MessageInputTextarea = ({
    ...
    return (
        <textarea
            ...
            onPaste={handlePaste}
            className={cn(
                "w-full sm:w-[80%] sm:min-w-[600px] p-4 sm:p-4 md:p-5 rounded-t-lg bg-background text-foreground resize-none text-sm min-h-[82px] max-h-[40vh] focus:outline-none placeholder:text-muted-foreground",
                "mx-2 sm:mx-2 md:mx-3",
                className,
            )}
...
>(({ className, children, ...props }, ref) => {
    ...
    const buttonClasses = cn(
        "w-10 h-10 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 flex items-center justify-center enabled:cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background",
        className,
    );
    ...
    return (
        <button
            ...
            data-slot={
                isPending ? "message-input-cancel" : "message-input-submit"
            }
            {...props}
        >
            {children ??
...
>(
    (
        ...
        return (
            <Tooltip
                content="Attach Files (Images, PDFs, Word Docs)"
                side="top"
            >
                <button
                    ref={ref}
