...
const MessageContent = React.forwardRef<HTMLDivElement, MessageContentProps>(
  (
    ...
    // 🔍 DEBUG: Log content rendering decisions
    React.useEffect(() => {
    ...
    }, [message.id, isLoading, hasContent, showLoading, message.reasoning, contentToRender]);
      ...
      >
        <div className="flex flex-col w-full">
          ...
          <div
            id={toolDetailsId}
            className={cn(
              "flex flex-col gap-1 p-3 pl-7 overflow-auto transition-[max-height,opacity,padding] duration-300 w-full break-words",
                ...
                {reasoningStep ? (
                  <div className="bg-muted/50 rounded-md p-3 text-xs overflow-x-auto overflow-y-auto max-w-full">
                    <div className="whitespace-pre-wrap break-words">
                      <MarkdownErrorBoundary
                        fallback={
                          <span className="whitespace-pre-wrap break-words">{reasoningStep}</span>
                        }
                      >
                        <Streamdown components={markdownComponents}>
                          {reasoningStep}
...
function formatToolResult(
  ...
  if (contentString) {
    try {
      ...
      return (
        <MarkdownErrorBoundary
          fallback={<span className="whitespace-pre-wrap break-words">{contentString}</span>}
        >
